angular.module('myApp').controller('EditSponsorController', 
  ['$scope', '$routeParams', '$location', '$translate', '$timeout', 'UserErrorService', 'NavigationService', 'EntityUtilService', 'SecurityService', 'SponsorService', 
  function($scope, $routeParams, $location, $translate, $timeout, UserErrorService, NavigationService, EntityUtilService, SecurityService, SponsorService) {
	...
	$scope.add = function () {
		...
		SponsorService.add(dataToServer($scope.obj)).then(function (httpResponse) {
			if($scope.parent) {
	...
	$scope.update = function () {
		$scope.uiWorking = true;
		SponsorService.update(dataToServer($scope.obj)).then(function () { //httpResponse
				returnBack();
	...
	$scope.delete = function () {
		$scope.uiWorking = true;
		SponsorService.delete($scope.obj._id).then(returnBack, errorHandlerDelete, progressNotify);
	};
	...
	$scope.gotoEdit = function() {
		$location.path('/sponsor/edit/' + $routeParams.id);		
	};

	$scope.gotoDelete = function() {
		$location.path('/sponsor/delete/' + $routeParams.id);		
	};
	...
	function init() {
		...
		if ($routeParams.id) {
			SponsorService.getDocument($routeParams.id).then(loadData, errorHandlerLoad);		
		}
