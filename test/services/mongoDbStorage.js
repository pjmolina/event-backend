...
function init(done) {
	var dbServer = (process.env.DB_SERVER) ? process.env.DB_SERVER : "localhost:27017"; 
	var dbCnx = "mongodb://" + dbServer + "/eventBackend_BinaryTest";
	mongoose.createConnection(dbCnx);
	cnx = mongoose.connect(dbCnx, {}, function(db) {
		sut.apply(cnx);
		done();
	});
}
